const sessionClient = class Session {
  static session = new Array();
  static pathSessionName = 'sessions';
  ...
  static async addInfoSession(nameSession: string, exted: object) {
    nameSession = this.replaceNameSession(nameSession);
  ...
  static async getSession(nameSession: string) {
    nameSession = this.replaceNameSession(nameSession);
    if (nameSession) {
      const check = await this.checkClient(nameSession);
      if (check) {
